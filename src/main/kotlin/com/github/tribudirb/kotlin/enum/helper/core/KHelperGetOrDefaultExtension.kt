...
inline fun <KEY, reified TYPE> KClass<TYPE>.getOrDefault(
    keys: Collection<KEY>,
    default: TYPE
): List<TYPE> where KEY : Any, TYPE : EnumHelper<KEY> = EnumHelper.run { getOrDefault(enum, keys, default) }
...
inline fun <KEY, reified TYPE> KClass<TYPE>.getOrDefault(
    key: KEY,
    default: TYPE
): TYPE where KEY : Any, TYPE : EnumHelper<KEY> = EnumHelper.run { getOrDefault(enum, key, default) }
...
inline fun <KEY, reified TYPE> KClass<TYPE>.getOrDefault(
    λ: Supplier<KEY>,
    default: TYPE
): TYPE where KEY : Any, TYPE : EnumHelper<KEY> = EnumHelper.run { getOrDefault(enum, λ(), default) }
...
inline fun <KEY, reified TYPE> KClass<TYPE>.getOrDefault(
    key: KEY,
    λ: Supplier<TYPE>
): TYPE where KEY : Any, TYPE : EnumHelper<KEY> = EnumHelper.run { getOrDefault(enum, key, λ()) }